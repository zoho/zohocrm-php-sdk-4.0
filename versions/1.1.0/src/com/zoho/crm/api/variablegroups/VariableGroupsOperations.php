...
class VariableGroupsOperations
{
	...
	public  function getVariableGroups()
	{
		...
		$apiPath=""; 
		$apiPath=$apiPath.('/crm/v4/settings/variable_groups'); 
	 ...
	 * @param string $variableGroupId A string
	 * @return APIResponse An instance of APIResponse
	 */
	public  function getVariableGroupById(string $variableGroupId)
	{
		$handlerInstance=new CommonAPIHandler(); 
		$apiPath=""; 
		$apiPath=$apiPath.('/crm/v4/settings/variable_groups/'); 
		$apiPath=$apiPath.(strval($variableGroupId)); 
		$handlerInstance->setAPIPath($apiPath); 
		$handlerInstance->setHttpMethod(Constants::REQUEST_METHOD_GET); 
		$handlerInstance->setCategoryMethod(Constants::REQUEST_CATEGORY_READ); 
		return $handlerInstance->apiCall(ResponseHandler::class, 'application/json'); 
